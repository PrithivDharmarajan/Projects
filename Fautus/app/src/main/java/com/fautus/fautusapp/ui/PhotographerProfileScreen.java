...
import static com.fautus.fautusapp.R.string.dob;
 ...
 */
public class PhotographerProfileScreen extends BaseActivity implements InterfaceBtnCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.ui_create_profile_screen);
        initView();
    }
    ...
    private void initView() {
        ...
        mHeaderTxt.setText(getString(R.string.create_your_profile).toUpperCase(Locale.US));
        ...
        switch (v.getId()) {
            ...
            case R.id.upload_selfie_holder_img:
                ...
                if (permissionsCameraAndStorage()) {
                    DialogManager.getInstance().showPictureUploadPopup(this, new InterfaceTwoBtnCallback() {
                        @Override
    ...
    private void validateFields() {
        ...
        if (fullNameStr.isEmpty()) {
            ...
            DialogManager.getInstance().showAlertPopup(this, getString(R.string.req_name_topic), getString(R.string.req_name_msg), PhotographerProfileScreen.this);
        } else if (cityStr.isEmpty()) {
            mCityEdt.requestFocus();
            DialogManager.getInstance().showAlertPopup(this, getString(R.string.req_city_topic), getString(R.string.req_city_msg), PhotographerProfileScreen.this);
        } else if (stateStr.isEmpty()) {
            mStateEdt.requestFocus();
            DialogManager.getInstance().showAlertPopup(this, getString(R.string.req_state_topic), getString(R.string.req_state_msg), PhotographerProfileScreen.this);
        } else if (languageSpokenStr.isEmpty()) {
            mLanguageSpokenEdt.requestFocus();
            DialogManager.getInstance().showAlertPopup(this, getString(R.string.req_language_topic), getString(R.string.req_language_msg), PhotographerProfileScreen.this);
        } else if (dobStr.isEmpty() || dobStr.equals(getString(dob))) {
            mDobTxt.requestFocus();
            DialogManager.getInstance().showAlertPopup(this, getString(R.string.req_dob_topic), getString(R.string.req_dob_msg), PhotographerProfileScreen.this);
        } else if (ssnStr.isEmpty()) {
            mSsnEdt.requestFocus();
            DialogManager.getInstance().showAlertPopup(this, getString(R.string.req_ssn_topic), getString(R.string.req_ssn_msg), PhotographerProfileScreen.this);
        } else if (ssnStr.length() != 4) {
            mSsnEdt.requestFocus();
            mSsnEdt.setSelection(ssnStr.length());
            DialogManager.getInstance().showAlertPopup(this, getString(R.string.req_ssn_topic), getString(R.string.req_ssn_4_digits_msg), this);
            ...
            DialogManager.getInstance().showAlertPopup(this, getString(R.string.req_selfie_topic), getString(R.string.req_selfie_msg), PhotographerProfileScreen.this);
            ...
            DialogManager.getInstance().showAlertPopup(this, getString(R.string.req_photo_id_topic), getString(R.string.req_photo_id_msg), PhotographerProfileScreen.this);
        } else {
            /*Parse API call*/
            if (mParseProfileObject == null) {
                mParseProfileObject = new ParseObject(ParseAPIConstants.Parse_Photographer);
            }
            mParseProfileObject.put(ParseAPIConstants.user, ParseUser.getCurrentUser());
            ...
            mParseProfileObject.put(ParseAPIConstants.isAvailable, false);
    ...
    public void onParseRequestSuccess() {
        ...
        nextScreen(SelectEquipmentScreen.class, true);
    }
    ...
    private void showDatePickerDialog() {
        ...
        if (!dateOfBirthStr.isEmpty() && !dateOfBirthStr.equalsIgnoreCase(getString(dob))) {
    ...
    private DatePickerDialog.OnDateSetListener mDateSetListener = new DatePickerDialog
        ...
        public void onDateSet(DatePicker view, int year, int monthOfYear,
            ...
            if (mYear >= cal.get(Calendar.YEAR) && mMonth >= cal.get(Calendar.MONTH) && mDate >=
                ...
                DialogManager.getInstance().showToast(PhotographerProfileScreen.this, getString(R.string.select_past_date));
            } else {
    ...
    private void captureImage() {
        ...
        mPictureFileUri = (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) ? FileProvider.getUriForFile(this, getApplicationContext().getPackageName() + ".provider", getOutputMediaFile(MEDIA_TYPE_IMAGE)) : Uri.fromFile(getOutputMediaFile(MEDIA_TYPE_IMAGE));
        ...
        if (requestCode == CAMERA_CAPTURE_IMAGE_REQUEST_CODE) {
            if (resultCode == RESULT_OK) {
                ...
                /*Cancelling the image capture process by the user*/
                    DialogManager.getInstance().showToast(this, getString(R.string.user_cancelled));
                } else {
               /*image capture getting failed due to certail technical issues*/
                    DialogManager.getInstance().showToast(this, getString(R.string.sorry_failed_img));
                }
            }
            ...
            if (resultCode == RESULT_OK) {
                ...
                Cursor cursor = getContentResolver().query(mPictureFileUri, null, null, null, null);
    ...
    private boolean permissionsCameraAndStorage() {
        ...
        if (!listPermissionsNeeded.isEmpty()) {
            addPermission = askAccessPermission(listPermissionsNeeded, new InterfaceTwoBtnCallback() {
                @Override
                public void onYesClick() {
                    DialogManager.getInstance().showPictureUploadPopup(PhotographerProfileScreen.this, new InterfaceTwoBtnCallback() {
                        @Override
                        public void onYesClick() {
                            captureImage();
                        }
                        ...
                        mParseProfileObject.getParseFile(ParseAPIConstants.photoIDPhoto).getDataInBackground(new GetDataCallback() {
                            ...
                            public void done(byte[] data, ParseException e) {
                                ...
                                try {
                                    ...
                                    mUploadPhotoIdTxt.setVisibility(View.INVISIBLE);
                                    mUploadPhotoCloseImg.setVisibility(View.VISIBLE);
                                    Glide.with(PhotographerProfileScreen.this)
                                            .load(data)
    ...
    private void updatePhotographerDetails() {
        ...
        mParseProfileObject.saveInBackground(new SaveCallback() {
            ...
            public void done(ParseException e) {
                DialogManager.getInstance().hideProgress();
                if (e == null) {
                    if (NetworkUtil.isNetworkAvailable(PhotographerProfileScreen.this)) {
                        ...
                        APIRequestHandler.getInstance().paresSaveInBackground(photographerUser, PhotographerProfileScreen.this);
                    } else {
                        DialogManager.getInstance().showAlertPopup(PhotographerProfileScreen.this, getString(R.string.app_name), getString(R.string.no_internet), PhotographerProfileScreen.this);
                    }
                } else {
                    DialogManager.getInstance().showAlertPopup(PhotographerProfileScreen.this, getString(R.string.app_name), e.getMessage(), PhotographerProfileScreen.this);
                }
            }
        });
    }
