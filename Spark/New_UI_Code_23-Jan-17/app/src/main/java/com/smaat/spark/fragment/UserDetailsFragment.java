    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.block_btn_lay:
                addAndDelFriendAPICall(AppConstants.API_BLOCK_FRIEND, getResources().getString(R.string.yes), getResources().getString(R.string.cancel),getString(R.string.block_friend));
...
//                    nextScreen(ChatNormalScreen.class, false);
                    ...
                    if (AppConstants.CHAT_USER_BACK.equals(AppConstants.SUCCESS_CODE)) {
                        getActivity().onBackPressed();
                    } else {
                        ((HomeScreen) getActivity()).replaceFragment(new ChatFriendFragment(), 1);
                    }

                } else if (addDelReqStr.equalsIgnoreCase(getString(R.string.accept_req))) {
                    AppConstants.CHAT_FRIEND_FROM_CONNECT = AppConstants.FAILURE_CODE;
                    ChatSendReceiveInputEntity friendListInputEntityRes = new ChatSendReceiveInputEntity(AppConstants.API_ACCEPT_REQUEST, AppConstants.PARAMS_ACCEPT_REQUEST, GlobalMethods.getUserID(getActivity()), mUserDetailsArrRes.getUser_id(), AppConstants.SUCCESS_CODE);
                    APIRequestHandler.getInstance().callAddFriendAPI(friendListInputEntityRes, this);
                ...
                if (ignoreDelStr.equalsIgnoreCase(getString(R.string.ignore))) {
                    ...
                    DialogManager.getInstance().showOptionPopup(getActivity(), getResources().getString(R.string.ignore_friend), getString(R.string.ignore), getString(R.string.cancel), new InterfaceTwoBtnCallback() {
                        ...
                        public void onYesClick() {
                            ChatSendReceiveInputEntity friendListInputEntityRes = new ChatSendReceiveInputEntity(AppConstants.API_IGNORE_FRIEND, AppConstants.PARAMS_IGNORE_FRIEND, GlobalMethods.getUserID(getActivity()), mUserDetailsArrRes.getUser_id(), AppConstants.FAILURE_CODE, "");
                            APIRequestHandler.getInstance().callAddFriendAPI(friendListInputEntityRes, UserDetailsFragment.this);
                        }
                    ...
                    addAndDelFriendAPICall(AppConstants.API_REMOVE_FRIEND, getResources().getString(R.string.delete), getResources().getString(R.string.cancel),getResources().getString(R.string.delete_friend));
                } else if (ignoreDelStr.equalsIgnoreCase(getString(R.string.ignored))) {
                    ChatSendReceiveInputEntity friendListInputEntityRes = new ChatSendReceiveInputEntity(AppConstants.API_IGNORE_FRIEND, AppConstants.PARAMS_IGNORE_FRIEND, GlobalMethods.getUserID(getActivity()), mUserDetailsArrRes.getUser_id(), AppConstants.SUCCESS_CODE, "");
                    APIRequestHandler.getInstance().callAddFriendAPI(friendListInputEntityRes, UserDetailsFragment.this);
                }
                break;
    ...
    private void addAndDelFriendAPICall(final String callApiNameStr, String buttonOneStr,String buttonTwoStr, String alertMsgStr) {
        DialogManager.getInstance().showOptionPopup(getActivity(), alertMsgStr, buttonOneStr, buttonTwoStr, new InterfaceTwoBtnCallback() {
            @Override
            public void onYesClick() {
    ...
    public void onRequestSuccess(Object resObj) {
        ...
        if (resObj instanceof CommonResponse) {
            ...
            if (addFriendRes.getResponse_code().equalsIgnoreCase(AppConstants.SUCCESS_CODE)) {
                ...
                String alertMsgStr = addFriendRes.getMessage();
                if (mIgnoreDelBtn.getText().toString().trim().equalsIgnoreCase(getString(R.string.del_friend))) {
//                    ChatReceiveEntity deleteEntityRes = new ChatReceiveEntity();
//                    deleteEntityRes.setUser_id(GlobalMethods.getUserID(getActivity()));
//                    deleteEntityRes.setFriend_id(mUserDetailsArrRes.getUser_id());
//                    deleteEntityRes.setSubject(AppConstants.CHAT_SUB_FRIEND);
//                    ChatTable.deleteChatMessageList(deleteEntityRes);
                    alertMsgStr = String.format(getResources().getString(R.string.deleted_friend), mUserDetailsArrRes.getUsername());
                }
                DialogManager.getInstance().showAlertPopup(getActivity(), alertMsgStr);
                getActivity().onBackPressed();
    ...
    private void setUserData() {
        ...
        mAddressTxt.setText(mUserDetailsArrRes.getHide_location().equals(AppConstants.SUCCESS_CODE) ? "" : mUserDetailsArrRes.getAddress());
        ...
        if (mUserDetailsArrRes.getFriend().equalsIgnoreCase(AppConstants.FAILURE_CODE)) {
            ...
            ignoreDelStr = getString(R.string.ignore);
        } else if (mUserDetailsArrRes.getFriend().equalsIgnoreCase(getResources().getString(R.string.two))) {
            addDelReqStr = getString(R.string.accept_req);
            ignoreDelStr = getString(R.string.ignore);
        } else if (mUserDetailsArrRes.getFriend().equalsIgnoreCase(getResources().getString(R.string.four))) {
            addDelReqStr = getString(R.string.add_friend);
            ignoreDelStr = getString(R.string.ignored);
        }

        sysOut("mUserDetailsArrRes.getFriend()---" + mUserDetailsArrRes.getFriend());
        mBlockBtnLay.setVisibility(mUserDetailsArrRes.getFriend().equalsIgnoreCase(getResources().getString(R.string.three)) ? View.INVISIBLE : View.VISIBLE);
