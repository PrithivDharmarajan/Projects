...
public class NotificationFragment extends BaseFragment implements InterfaceBtnCallback {
        ...
        if (resObj instanceof NotificationResponse) {
            ...
            if (addFriendRes.getResponse_code().equalsIgnoreCase(AppConstants.SUCCESS_CODE)) {
                if (addFriendRes.getResult().size() > 0) {
                    ...
                    DialogManager.getInstance().showAlertPopup(getActivity(), getResources().getString(R.string.no_notification_msg));
    ...
    private void setNotificationAdapter(ArrayList<UserDetailsEntity> notificationList) {

        NotificationAdapter adapter = new NotificationAdapter(getActivity(), this, notificationList);
