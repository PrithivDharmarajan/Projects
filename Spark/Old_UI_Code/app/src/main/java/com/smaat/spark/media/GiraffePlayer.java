...
//            $.id(R.id.app_video_play).image(R.drawable.ic_play_arrow_white_24dp);
    ...
    private void showBottomControl(boolean show) {
        ...
        $.id(R.id.app_video_currentTime).visibility(show ? View.GONE : View.GONE);
        $.id(R.id.app_video_endTime).visibility(show ? View.GONE : View.GONE);
        $.id(R.id.video_seek_bar).visibility(show ? View.VISIBLE : View.VISIBLE);
    }
...
//                        Toaster.show("download rate:" + extra);
        ...
        seekBar = (SeekBar) activity.findViewById(R.id.video_seek_bar);
        seekBar.setMax(1000);
    ...
    private void statusChange(int newStatus) {
        ...
        if (!isLive && newStatus == STATUS_COMPLETED) {
            ...
            if (mStatusHandler != null) {
                ...
                $.id(R.id.video_seek_bar).visibility(View.GONE);
                mStatusHandler.removeCallbacks(showSeekBar);
            }
            $.id(R.id.app_video_replay).gone();
            ...
            if (mStatusHandler != null) {
                ...
                $.id(R.id.video_seek_bar).visibility(View.GONE);
                mStatusHandler.removeCallbacks(showSeekBar);
                mStatusHandler.postDelayed(showSeekBar, 0);
            }
        }

    }

    private Runnable showSeekBar = new Runnable() {
        @Override
        public void run() {
            $.id(R.id.app_video_bottom_box).visibility(View.VISIBLE);
            $.id(R.id.video_seek_bar).visibility(View.VISIBLE);
            setProgress();
            mStatusHandler.postDelayed(this, 0);
...
//            $.id(R.id.app_video_fullscreen).image(R.drawable.ic_fullscreen_white_24dp);
    ...
    public void setShowNavIcon(boolean show) {
        $.id(R.id.app_video_finish).visibility(show ? View.GONE : View.GONE);
    }
