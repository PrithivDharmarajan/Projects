 ...
 */
public class UploadDropboxPreviewFragment extends Fragment implements DialogMangerOkCallback {
...
//    private DbxChooser mChooser;
    ...
    public void onResume() {
        super.onResume();
        if (UploadScreen.mEntity.dropBoxUrlPreview != null && UploadScreen.mEntity.dropBoxUrlPreview.length() > 0) {
            mInputBox.setText(UploadScreen.mEntity.dropBoxUrlPreview);
        }
    }
        ...
        mNextBtn.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                if (mInputBox.getText().toString().trim().length() > 0) {
                    UploadScreen.mEntity.dropBoxUrlPreview = mInputBox.getText().toString().trim();
                    UploadScreen.mEntity.isDropBoxUrl = false;
//                    ((BaseFragmentActivity) getActivity()).addFragment(new UploadDropboxOrginalFragment(), 11);
                    ((BaseFragmentActivity) getActivity()).addFragment(new UploadDropboxOrginalFragment(), 10);
                } else {
                    DialogManager.showBasicAlertDialog(getActivity(), getString(R.string.enter_url), UploadDropboxPreviewFragment.this);
