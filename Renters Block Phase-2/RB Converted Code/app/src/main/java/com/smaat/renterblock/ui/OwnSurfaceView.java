 ...
 */
public class OwnSurfaceView extends SurfaceView implements
	...
	Camera.PreviewCallback cameraPreviewCallback = new Camera.PreviewCallback() {
		...
		public void onPreviewFrame(byte[] data, Camera camera) {
			...
			processCameraData(data, params.getPreviewSize().width,
	...
	public void openCamera() {
		...
		final List<int[]> supportedPreviewFpsRange = parameters
		...
		if (frameSize != null) {
			...
			int lastElementWidth = supportedPreviewSizes
					.get(supportedPreviewSizes.size() - 1).width;
			Camera.Size minPreviewSize = (lastElementWidth > firstElementWidth) ? supportedPreviewSizes
					.get(0) : supportedPreviewSizes.get(supportedPreviewSizes
					.size() - 1);
	...
	private void processCameraData(final byte[] cameraData,
		...
		boolean offerSuccess = cameraPreviewCallbackQueue.offer(new Runnable() {
			...
			public void run() {
				...
				YuvImage image = new YuvImage(cameraData, ImageFormat.NV21,
				...
				// rotate image
				byte[] rotatedCameraData = rotateImage(jpegVideoFrameData,
						imageWidth, imageHeight, currentCameraId);
	...
	private byte[] rotateImage(byte[] cameraData, final int imageWidth,
			final int imageHeight, int currentCameraId) {
		Bitmap landscapeCameraDataBitmap = BitmapFactory.decodeByteArray(
				cameraData, 0, cameraData.length);

		Bitmap portraitBitmap = null;
		if (currentCameraId == getCameraId(Camera.CameraInfo.CAMERA_FACING_FRONT)) { // front
																						// camera
			portraitBitmap = Bitmap.createBitmap(landscapeCameraDataBitmap, 0,
					0, imageWidth, imageHeight, rotationMatrixFront, true);
		} else { // back camera
			portraitBitmap = Bitmap.createBitmap(landscapeCameraDataBitmap, 0,
					0, imageWidth, imageHeight, rotationMatrixBack, true);
		}

		landscapeCameraDataBitmap.recycle();
		ByteArrayOutputStream stream = new ByteArrayOutputStream();
		if (!portraitBitmap.isRecycled()) {
			portraitBitmap.compress(Bitmap.CompressFormat.JPEG, IMAGE_QUALITY,
					stream);
